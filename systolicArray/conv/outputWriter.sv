    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (ow_state)
                ...
                OW_WRITE: begin
                    $display("[OW @%0t] WRITE cur_row=%0d eff_rows=%0d row_start=%0d word_addr=%0d rq_out=%0d",
                        $time, cur_row, lat_eff_rows, row_start, word_addr, $signed(rq_out[0]));
                    begin
                        int sl;
                        sl = int'(row_start) % 4;
                        overflow_word <= '0;
    ...
    always_comb begin
        ...
        if (ow_state == OW_WRITE || ow_state == OW_OVERFLOW) begin
            ...
            if (is_last_layer) begin
                ...
                $display("[OW @%0t] EXT_Z_WR addr=%0d data=0x%08x mask=%04b",
                    $time, ow_waddr, ow_word, ow_mask);
