...
// compute_engine.sv
//
// Everything except dram_loader and bias_loader lives here.
// Absorbed modules:
//   tile_iter           ? tile coordinate generator
//   master FSM          ? top-level sequencer (was syscontroller)
//   SRAM port-0 mux     ? arbitrates dram_loader vs compute write
//   sram_shadow_reader  ? drains SRAM into internal register files
//   feed_sequencer      ? skews data into the systolic array
//   systolic_tiled      ? 4?4 MAC grid
//   output_writer       ? packs results, writes to DRAM Z or SRAM A/C
//
// All intermediate signals (x_rf, y_rf, a_feed, b_feed, tile_z, local_rst,
// tile geometry between stages) are fully internal.
//
// Master FSM:
//   MS_RESET        ? fire bias_loader start + tile_iter init
//   MS_LOAD_BIAS    ? wait for bias_loader done
//   MS_LOAD_DRAM    ? fire dram_loader start, wait for load_ready
//   MS_READ_A       ? drain SRAM activations into x_rf
//   MS_READ_B       ? drain SRAM weights into y_rf
...
//   MS_ADVANCE      ? pulse tile_iter advance
//   MS_ADVANCE_CHK  ? wait 1 cycle for tile_iter to settle
//   MS_ADVANCE_CHK2 ? check all_done, loop or finish
...
module compute_engine #(
    ...
    input  logic act_sel,    // 0 = activations from SRAM A, 1 = from SRAM C
    input  logic out_sel,    // 0 = intermediate to SRAM C, 1 = to SRAM A
    input  logic skip_x,     // 1 = skip activation fetch (already in SRAM)
    ...
    // Tile geometry outputs to dram_loader
    ...
    // -------------------------------------------------------------------------
    // SRAM port 0 ? shared with dram_loader (mux inside here)
    // dram_loader drives port 0 during MS_LOAD_DRAM
    // compute_engine drives port 0 during MS_WRITE / MS_OVERFLOW
    // -------------------------------------------------------------------------

    // From dram_loader
    input  logic          dl_sram_a_csb0,
    input  logic          dl_sram_a_web0,
    input  logic [3:0]    dl_sram_a_wmask0,
    input  logic [AW-1:0] dl_sram_a_addr0,
    input  logic [31:0]   dl_sram_a_din0,

    input  logic          dl_sram_b_csb0,
    input  logic          dl_sram_b_web0,
    input  logic [3:0]    dl_sram_b_wmask0,
    input  logic [AW-1:0] dl_sram_b_addr0,
    input  logic [31:0]   dl_sram_b_din0,

    // To actual SRAMs ? muxed outputs
    ...
    // -------------------------------------------------------------------------
    // SRAM port 1 ? read only, owned entirely by compute_engine
    // -------------------------------------------------------------------------
    ...
    typedef enum logic [3:0] {
        ...
        MS_OVERFLOW     = 4'd9,
        MS_ADVANCE      = 4'd10,
        MS_ADVANCE_CHK  = 4'd11,
        MS_ADVANCE_CHK2 = 4'd12,
        MS_DONE         = 4'd13
    } ms_t;
    ms_t ms_state;

    // =========================================================================
    // Tile iterator state (was tile_iter module)
    // =========================================================================
    ...
    // Tile iterator helper functions
    ...
    // Effective rows/cols computed combinationally from current tile position
    ...
    // Wire tile geometry to dram_loader outputs
    ...
    assign dl_sram_a_base_o = AW'((int'(ti_row_tile) * REAL_M + 3) / 4);
    ...
    // =========================================================================
    // Internal systolic array feeds and results
    // =========================================================================
    ...
    // =========================================================================
    // Output bytes ? raw accumulator truncated to 8 bits
    // SIMD unit plugs in here when integrated
    // =========================================================================
    ...
    // =========================================================================
    // state_entry pulse ? fires one cycle after entering a new state
    // used to generate clean single-cycle start pulses to dram_loader
    // =========================================================================
    ...
    always_ff @(posedge clk or posedge rst) begin : ms_seq
        ...
        if (rst) begin
            ...
            case (ms_state)
                ...
                // -------------------------------------------------------------
                // Fire bias loader and initialise tile iterator
                // -------------------------------------------------------------
                MS_RESET: begin
                    bl_start_o  <= 1'b1;
                    state_entry <= 1'b1;
                ...
                // -------------------------------------------------------------
                // Fire dram_loader, wait for activation + weight load complete
                // -------------------------------------------------------------
                MS_LOAD_DRAM: begin
                    if (state_entry_d1) dl_start_o <= 1'b1;
                    if (dl_load_ready_i) begin
                        // Latch tile geometry for the compute pass
                ...
                // -------------------------------------------------------------
                // Assert local reset to systolic array ? clears accumulators
                // -------------------------------------------------------------
                ...
                // -------------------------------------------------------------
                // Deassert local reset ? array clear, ready to receive data
                // -------------------------------------------------------------
                MS_DEASSERT: begin
                    local_rst <= 1'b0;
                    run_cnt   <= '0;
                ...
                // -------------------------------------------------------------
                // Feed skewed data, count until all dot products complete
                // -------------------------------------------------------------
                MS_RUN: begin
                    local_rst <= 1'b0;
                ...
                // -------------------------------------------------------------
                // Advance tile iterator, check if all tiles done
                // -------------------------------------------------------------
                MS_ADVANCE: begin
                    // tile_iter advance happens combinationally in the
                    // tile_iter always_ff above when ms_state == MS_ADVANCE
                    ms_state <= MS_ADVANCE_CHK;
                end

                MS_ADVANCE_CHK: begin
                    ms_state <= MS_ADVANCE_CHK2;
                end

                MS_ADVANCE_CHK2: begin
    ...
    // =========================================================================
    // Combinational: feed sequencer skew logic
    // =========================================================================
    always_comb begin
    ...
    // =========================================================================
    // Combinational: SRAM port 1 read address generation
    // =========================================================================
    always_comb begin
    ...
    // =========================================================================
    // Combinational: SRAM port 0 mux + output write path
    //
    // During MS_LOAD_DRAM: dram_loader owns SRAM A port 0 (write activations)
    //                      dram_loader owns SRAM B port 0 (write weights)
    // During MS_WRITE / MS_OVERFLOW: compute_engine writes results to
    //                      SRAM C (out_sel=0) or SRAM A (out_sel=1)
    //                      or DRAM Z (is_last_layer=1)
    // =========================================================================
    logic [31:0]   ow_word;
    logic [3:0]    ow_mask;
    ...
    always_comb begin
        ...
        if (ms_state == MS_LOAD_DRAM) begin
            // dram_loader owns SRAM A and B port 0
