...
//             Previously both capture and toggle happened on conv_done in the
//             same always_ff. NBA scheduling in some simulators means
//             sram_sel_at_done could capture the POST-toggle value, causing
//             the copy FSM to read the wrong ping-pong bank (all-zeros/X).
...
//             This guarantees sram_sel_at_done is stable and correct for
//             the entire duration of the copy FSM run.
//
// FIX 3 (v5): cpy_locked_sel - eliminates residual race on even inferences.
//             sram_sel_at_done is a registered signal that is updated at every
//             inference boundary.  On even-numbered inferences sram_sel toggles
//             from 1->0 at T+1 (conv_done_d), and the combinational port-1
//             steering block plus the clocked simd_dmem write both sample
//             sram_sel_at_done with no timing guarantee relative to cpy_wr_en.
//             Fix: latch sram_sel_at_done into cpy_locked_sel the exact cycle
//             cpy_running goes high (T+0).  All copy-window logic then uses
//             cpy_locked_sel exclusively - a value that cannot change for the
//             entire 256+ cycle copy window regardless of subsequent toggles.
//
// Data flow:
...
//                             -> simd_done
//
// Input format : 8 x 8 x 4  (H x W x C, int8, NHWC)
// =============================================================================
...
module accelerator_top #(
    ...
    //                                 (the bank just written by the engine)
    //   T+1 : conv_done_d=1       -> sram_sel toggles (ready for next inference)
    //         cpy_running goes high (copy FSM starts reading cpy_locked_sel bank)
    //         cpy_locked_sel latched from sram_sel_at_done (frozen for copy window)
    //
    // Because cpy_locked_sel is latched at T+0 and never changes until the next
    // inference starts, the copy FSM always sees the correct bank throughout its
    // 256-cycle run, regardless of subsequent sram_sel or sram_sel_at_done changes.
    // =========================================================================
    logic sram_sel;           // which bank the engine is currently writing: 0=C  1=OUT
    logic sram_sel_at_done;   // bank written by the inference that just finished
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // Toggle ONE cycle AFTER capture
            if (conv_done_d)
                sram_sel <= ~sram_sel;
        end
    end
    ...
    //
    // Triggered by conv_done (not conv_done_d) so the copy starts immediately
    // when the inference finishes, giving the SIMD CPU maximum time.
    //
    // cpy_done fires the cycle after the last write (addr 255) completes.
    // =========================================================================
    logic [7:0] cpy_addr;        // SRAM read address (this cycle)
    logic [7:0] cpy_wr_addr;     // simd_dmem write address (1 cycle after read)
    logic       cpy_wr_en;       // write enable for simd_dmem
    logic       cpy_running;     // SRAM read phase active
    logic       cpy_done;        // single-cycle pulse: all 256 words written
    logic       conv_done_r;     // edge detect for copy FSM start
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (!cpy_running && !cpy_wr_en) begin
                // Idle: start on rising edge of conv_done
                if (conv_done && !conv_done_r) begin
                    cpy_addr       <= 8'd0;
                    cpy_wr_addr    <= 8'd0;
                    cpy_running    <= 1'b1;
                    // [v5] Latch bank selector NOW, before sram_sel can toggle
                    // at T+1 (conv_done_d).  cpy_locked_sel will not change
                    // again until the next inference's conv_done pulse.
                    cpy_locked_sel <= sram_sel_at_done;
                end

            end else if (cpy_running) begin
                // Read phase: advance SRAM read pointer each cycle.
                // Write pointer lags by 1 (cpy_wr_addr/en use previous cpy_addr).
                cpy_wr_en   <= 1'b1;
                cpy_wr_addr <= cpy_addr;

                if (cpy_addr == 8'd255) begin
                    cpy_running <= 1'b0;
                    // cpy_wr_en will fire once more (final write for addr 255)
                end else begin
                    cpy_addr <= cpy_addr + 8'd1;
                end

            end else if (cpy_wr_en) begin
                // Final write cycle: addr 255 result now arriving from SRAM
    ...
    always_comb begin
        ...
        if (cpy_running) begin
            if (cpy_locked_sel == 1'b0) begin   // [v5] was: sram_sel_at_done
                sram_c_csb1  = 1'b0;
    ...
    // Written by pipelined copy FSM and by CPUai_top STORE instructions.
    // =========================================================================
    logic [63:0] simd_dmem [0:SIMD_DMEM_DEPTH-1];

    logic [9:0]  simd_data_addr;
    logic [63:0] simd_data_rd;     // memory -> CPU (LOAD)
    logic [63:0] simd_data_wr;     // CPU -> memory (STORE)
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // instead of sram_sel_at_done to prevent bank mis-selection on
            // even-numbered inferences when sram_sel_at_done may have been
            // updated by a new conv_done before the last cpy_wr_en fires.
            if (cpy_wr_en) begin
                if (cpy_locked_sel == 1'b0)         // [v5] was: sram_sel_at_done
    ...
    // Defaults to HALT; TB writes the real program before releasing reset.
    // =========================================================================
    ...
    // =========================================================================
    // SIMD reset control
    //   Held in reset: at global rst OR after simd_done
    //   Released:      only after cpy_done (copy FSM finished writing simd_dmem)
    // =========================================================================
    logic simd_rst;
    ...
    systolic_pipe_conv #(
        ...
        // Engine's SRAM C port goes through the ping-pong MUX
...
//
// 7 test cases. Each prints:
//   - Inputs  : tensor shape, kernel, bias, config
//   - Outputs : first 20 int8 results from DRAM_Z
//   - Cycles  : conv cycles, SIMD cycles, total
//   - Time    : simulated wall-clock time (ns) at 100 MHz
//   - Speedup : HW total vs scalar SW reference
// =============================================================================

module tb_accelerator_top;

    // =========================================================================
    // DUT parameters ? fixed hardware configuration
    // =========================================================================
    ...
    //   x_seed : controls activation values  byte = (i * x_seed) % 127 + 1
    //   w_seed : controls weight values      byte = (i % w_mod)  + 1
    //   b_val  : fixed bias for all channels (int32)
    ...
    // Print performance box for one test
    task automatic print_perf(
        ...
        hw_ns     = real'(total_cyc) * 10.0;   // 10 ns per cycle @ 100 MHz
    ...
    // =========================================================================
    // Generic run task ? used by all 7 tests
    //   desc      : short description string
    //   xseed     : activation data seed
    //   wmod      : weight modulus
    //   bval      : bias int32 value
    //   use_relu  : act_sel value
    //   expect_pass: 1 to check no-negatives (ReLU tests), 0 otherwise
    // =========================================================================
    task automatic run_test(
        ...
        // Derived per-test values (all use same hardware shape)
        ...
        // Capture simulation time before and after inference
        // With `timescale 1ns/1ps, $time is in nanoseconds directly
        t_start_ns = $time;
        ...
        if (use_relu) begin
            ...
            // For non-ReLU tests just check done signals arrived
    ...
    initial begin
        ...
        // ------------------------------------------------------------------
        // TEST 1 ? Small positive activations, uniform weights, no activation
        //          Baseline: checks basic conv path end-to-end
        // ------------------------------------------------------------------
        run_test("Conv2D ? baseline (no activation)",
                 /*xseed*/1, /*wmod*/3, /*bval*/0, /*relu*/1'b0, 1);

        // ------------------------------------------------------------------
        // TEST 2 ? Same data with ReLU enabled
        //          Verifies SIMD ReLU clamps negatives to 0
        // ------------------------------------------------------------------
        run_test("Conv2D + ReLU ? clamp negatives to zero",
                 1, 3, 0, 1'b1, 2);

        // ------------------------------------------------------------------
        // TEST 3 ? Larger activations (high x_seed) + small weights
        //          Tests saturation of int8 accumulator
        // ------------------------------------------------------------------
        ...
        //          Complementary saturation test
        // ------------------------------------------------------------------
        run_test("Conv2D ? small activations, large weights",
                 1, 7, 0, 1'b0, 4);

        // ------------------------------------------------------------------
        // TEST 5 ? Positive bias applied (+50 per output channel)
        //          Output should shift upward
        // ------------------------------------------------------------------
        run_test("Conv2D + positive bias (+50)",
                 1, 3, 50, 1'b0, 5);

        // ------------------------------------------------------------------
        // TEST 6 ? Negative bias + ReLU
        //          Negative bias pulls values down; ReLU should clamp them
        // ------------------------------------------------------------------
        run_test("Conv2D + negative bias (-200) + ReLU",
                 1, 3, -200, 1'b1, 6);

        // ------------------------------------------------------------------
        // TEST 7 ? All maximum-value activations and weights
        //          Stress test: every byte = 127, every weight = 5
        //          Exercises max accumulator range
        // ------------------------------------------------------------------
