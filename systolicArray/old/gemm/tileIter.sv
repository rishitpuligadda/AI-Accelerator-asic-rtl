module tile_iter #(
    ...
    // Control
    input  logic start_i,    // begin iteration from tile (0,0)
    input  logic advance_i,  // move to next tile
    output logic valid_o,    // current tile outputs are valid
    output logic all_done_o, // all tiles have been processed

    // Current tile coordinates
    output logic [$clog2(REAL_K):0]        row_tile_o,
    output logic [$clog2(REAL_N):0]        col_tile_o,

    // Derived geometry for current tile
    output logic [$clog2(TILE_MAX+1)-1:0]  eff_rows_o,   // min(REAL_K - row_tile, TILE_MAX)
    output logic [$clog2(TILE_MAX+1)-1:0]  eff_cols_o,   // min(REAL_N - col_tile, TILE_MAX)
    output logic [5:0]                     words_a_o,    // ceil(eff_rows * REAL_M / 4)
    output logic [5:0]                     words_b_o,    // ceil(REAL_M * eff_cols / 4)
    output logic [AW-1:0]                  sram_a_base_o // SRAM A word offset for this tile's X
);
    ...
    // -------------------------------------------------------------------------
    // Derived geometry ? purely combinational from (row_tile, col_tile)
    // -------------------------------------------------------------------------
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (start_i) begin
                ...
                // Advance col first, then row (row-major)
